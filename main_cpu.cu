...
//  */
// thrust::host_vector<int> get_neighbors(const thrust::host_vector<vec2> &v) {
...
*/
void print_graph( thrust::host_vector<std::vector<vec2>> e, thrust::host_vector<vec2> f) {
...
/*
  ...
  where the edges at index i ( i.e. e[i] ) correspond to the point at f[i]
*/
void write_graph( thrust::host_vector<std::vector<vec2>> e, thrust::host_vector<vec2> f) {

  std::ofstream outfile;
  outfile.open("CPU_graph.txt", std::ofstream::trunc);
...
int main(int argc, char **argv)
{
  ...
  thrust::host_vector<float> dists_to_point(filter_target.size());

  // loop through the filtered points
  for(int i = 0; i < filter_target.size(); i++ ) {

    // save array so we can sort them each step
    thrust::host_vector<vec2> target_points = filter_target;
